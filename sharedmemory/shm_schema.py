...
# Phase K8 (P1-5): 촉각 (DEX3 press_sensor_state) on/off. off 면 기존 동작 100% 유지.
TACTILE_MAPPING    = {'off':        0, 'on':       1}
# Phase N (PART4): lower-body 모드. 'hoist' = rt/lowcmd 전체 직접 제어 (호이스트 전제),
# 'loco' = motion mode (rt/arm_sdk, 상체만, 내장 LocoClient 가 밸런싱/보행).
LOWER_BODY_MAPPING = {'hoist':      0, 'loco':     1}
...
TACTILE_MAPPING_INV    = {v: k for k, v in TACTILE_MAPPING.items()}
LOWER_BODY_MAPPING_INV = {v: k for k, v in LOWER_BODY_MAPPING.items()}
...
TELEOP_CONFIG = [
    ...
    ("tactile_mode",(), np.int32),  # TACTILE_MAPPING     (0=off, 1=on) Phase K8
    ("lower_body",  (), np.int32),  # LOWER_BODY_MAPPING  (0=hoist, 1=loco) Phase N
]
