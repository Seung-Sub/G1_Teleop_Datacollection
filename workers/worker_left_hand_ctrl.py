...
from hand_control.robot_hand_inspire_single import Inspire_Controller_Left
...
def worker_left_hand_ctrl(shared_event, shm_name, shared_lock):
            ...
            if state is State.WAIT_CONNECT:
                ...
                if shared_event['set_hand'].is_set() and not hand_initialized:
                    try:
                        ...
                        dual_hand_state_array = Array('d', 6, lock = False)   # [output] current left hand state(6) data.
                        dual_hand_action_array = Array('d', 6, lock = False)  # [output] current left hand action(6) data.
                        hand_ctrl = Inspire_Controller_Left(shared_event,shm_name, shared_lock,left_hand_array, \
                                                    dual_hand_data_lock, dual_hand_state_array, dual_hand_action_array, \
                                                        fps = 100.0, Unit_Test = False)
    
                        hand_initialized = True
                        logger_mp.info("[Left_Hand_Ctrl] Hand controller initialized.")
                ...
                if not shared_event['set_start'].is_set():
                    left_q_target  = np.array([1,1,1,1,1,1])
                    hand_ctrl.ctrl_left_hand(left_q_target=left_q_target)


            elif state is State.RUN:

                mode = record_mode_shm.read_data()
                home, replay, deploy, start = mode["home"], mode["replay"], mode["deploy"], mode["start"]
                ...
                if shared_event['set_start'].is_set() and not replay and not deploy: 
                    ...
                    # Create a zero-filled array of size 12 for dual-hand compatibility
                    obs_hand_full = np.zeros(12)
                    obs_hand_full[:6] = dual_hand_state_array[:]
                    robot_obs_shm.write_data(
                        obs_hand=obs_hand_full,
                    )

                    action_hand_full = np.zeros(12)
                    action_hand_full[:6] = dual_hand_action_array[:]
                    robot_action_shm.write_data(
                        action_hand=action_hand_full,
                    )
                ...
                if deploy and shared_event['set_start'].is_set(): 
