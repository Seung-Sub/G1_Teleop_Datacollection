...
# Reduced Joint Names (22 dimensions for both state and action)
REDUCED_JOINT_NAMES = [
    # Waist (3)
    ...
    for col, dt in dtypes.items():
        ...
        if col in ("observation.state", "action"):
                ...
                try:
                    ...
                    if arr.size > 1:
                        ...
                        if col == "observation.state":
                            joint_names = REDUCED_JOINT_NAMES[:target_len]
                        elif col == "action":
                            joint_names = REDUCED_JOINT_NAMES[:target_len]
                except Exception:
                    pass
            ...
            if target_len is None:
                if col == "observation.state":
                    target_len = len(REDUCED_JOINT_NAMES)
                    joint_names = REDUCED_JOINT_NAMES
                elif col == "action":
                    target_len = len(REDUCED_JOINT_NAMES)
                    joint_names = REDUCED_JOINT_NAMES
...
def step6_copy_modality_json(dataset_root: Path):
    """
    Copy kistar_modality_reduced.json from utils/parquet/ to dataset meta folder as modality.json.
    """
    script_dir = Path(__file__).parent
    source_modality = script_dir / "kistar_modality_reduced.json"
    target_modality = dataset_root / "meta" / "modality.json"
    
    if source_modality.exists():
        import shutil
        shutil.copy2(source_modality, target_modality)
        print(f"[6] ✅ kistar_modality_reduced.json → modality.json 복사: {target_modality}")
    else:
        print(f"[6] ⚠️ kistar_modality_reduced.json을 찾지 못했습니다: {source_modality}")
...
def main():
    ...
    parser.add_argument("--view-keys", type=str, nargs='+', 
                        default=["observation.images.ego_left_view_masked", "observation.images.ego_right_view_masked"],
                        help="Video view keys under videos/chunk-xxx/<VIEW_KEY> (supports multiple)")
    parser.add_argument("--primary-view-key", type=str, default="observation.images.ego_left_view_masked",
