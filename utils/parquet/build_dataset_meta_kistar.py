...
def step6_copy_modality_json(dataset_root: Path):
    """
    Copy kistar_modality.json from utils/parquet/ to dataset meta folder as modality.json.
    """
    script_dir = Path(__file__).parent
    source_modality = script_dir / "kistar_modality.json"
    target_modality = dataset_root / "meta" / "modality.json"
    
    if source_modality.exists():
        import shutil
        shutil.copy2(source_modality, target_modality)
        print(f"[6] ✅ kistar_modality.json → modality.json 복사: {target_modality}")
    else:
        print(f"[6] ⚠️ kistar_modality.json을 찾지 못했습니다: {source_modality}")
