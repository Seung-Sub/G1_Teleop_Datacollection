...
def IK_Arm_np(
    ...
    return (q + delta_q).astype(np.float32)


def lerp(q0: np.ndarray, q1: np.ndarray, s: float) -> np.ndarray:
...
def quintic_interp(q0: np.ndarray, q1: np.ndarray, s: float) -> np.ndarray:
