...
def Transform_Wrist2Hand_V2_np(
    ...
    # t = np.array([0.0, -0.0235, -0.0972], dtype=np.float32)
    t = np.array([0.0, -0.0085, -0.1072], dtype=np.float32)

    # pos_hand = pos_wrist - R_hand.dot(t)
    pos_hand = pos_wrist - R_hand.dot(t)

    return quat_hand.astype(np.float32), pos_hand.astype(np.float32)
...
def FK_Thumb_V2_np(
    ...
    Finger_base = np.array([-0.01423, -0.0145, -0.04425], dtype=np.float32)
    ...
    MP_to_Tip = link_2 * right_direction + rotate_vector(IP_to_Tip, -joint_angle[3], flexion_axis)

    MP_rot_to_Tip = MP_interm_link * right_direction + rotate_vector(MP_to_Tip, -joint_angle[2], flexion_axis)

    CMC_to_Tip = (link_1 - MP_interm_link) * right_direction + rotate_vector(MP_rot_to_Tip, -joint_angle[1], roll_axis)

    Thumb_base_to_Tip = rotate_vector(CMC_to_Tip, -joint_angle[0], flexion_axis)
    ...
    return contact_point.astype(np.float32)
...
def FK_Finger_V2_np(
    ...
    if finger_type == "Index":
        Finger_base = np.array([-0.02924, -0.00895, -0.01078], dtype=np.float32)
        finger_deviation = -5.0
    elif finger_type == "Middle":
        Finger_base = np.array([0.0, -0.00895, -0.0095], dtype=np.float32)
    ...
    horiz_rot_axis = np.array([-1.0, 0.0, 0.0], dtype=np.float32)
    verti_rot_axis = np.array([0.0, 1.0, -0.0], dtype=np.float32)

    # Link transformations
    DIP_to_Tip = link_3 * upright_direction

    PIP_to_Tip = link_2 * upright_direction + rotate_vector(DIP_to_Tip, joint_angles[3], horiz_rot_axis)

    MCP_to_Tip = link_1 * upright_direction + rotate_vector(PIP_to_Tip, joint_angles[2], horiz_rot_axis)

    MCP_base_to_Tip = link_0 * upright_direction + rotate_vector(MCP_to_Tip, joint_angles[1], horiz_rot_axis)

    MCP_origin_to_Tip = rotate_vector(MCP_base_to_Tip, joint_angles[0], verti_rot_axis)
