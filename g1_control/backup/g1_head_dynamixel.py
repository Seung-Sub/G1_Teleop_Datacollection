...
class Dynamixel_Controller:
    def __init__(self):
        ...
        self.q_target = np.array(Custom_Angle)  # pitch 모터를 더 아래쪽으로 조정
    ...
    def _subscribe_motor_state(self):
        ...
        while self.running:
            ...
            if now - t_start >= 1.0:
                ...
                #다이나믹셀 실시간 각도 측정
                current_q_values = [ms.q for ms in lowstate.motor_state]
                #logger_mp.info(f"DEBUG - Current Dynamixel Ticks: {current_q_values}")
                loop_count = 0
                t_start = now
    ...
    def ctrl_dynamixel(self, q_target):
        ...
        with self.ctrl_lock:
            ...
            # 입력값 무시하고 항상 고정값 사용
            self.q_target = np.array(Custom_Angle, dtype=int)
...
#----------------------------------------------------------------------#
    ...
    def ctrl_dynamixel_go_home(self):
        ...
        with self.ctrl_lock:
            self.q_target = np.array(Custom_Angle)  # pitch 모터를 고정 값으로 설정
