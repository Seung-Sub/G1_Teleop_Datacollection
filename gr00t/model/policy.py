...
class Gr00tPolicy(BasePolicy):
    ...
    def get_attention_map(self, observations: Dict[str, Any]):
        """
        Return only the backbone attention maps for the given observations.
        """
        # 1) batch dimension
        is_batch = self._check_state_is_batched(observations)
        if not is_batch:
        ...
        # 4) prepare_input -> move to device & dtype, then pass through backbone
        backbone_inputs, _ = self.model.prepare_input(normalized_input)
        backbone_outputs = self.model.backbone(backbone_inputs)

        # 5) extract attention maps
        attn_maps = backbone_outputs.get("backbone_attention_mask", None)

        # 6) remove batch dimension if needed
        if attn_maps is not None and not is_batch:
            attn_maps = tuple(a.squeeze(0) for a in attn_maps)
